...
def print_menu_studio(studio):
    ...
    while not salir:
        print("********" + str(studio.id) + "********")
        """Tendremos que añadir películas en primer lugar, no se podrán añadir personajes si no están agregadas las 
        películas relacionadas y tampoco se podrán añadir vehículos hasta que no esté la película y el piloto (
        personaje) relacionado en el sistema """
...
def search_film(studio):
    ...
    resultado_busqueda = search_film_studio_dict(studio.id, studio_dict, title=title, release_year=release_year,
                                                 director=director)
